...
- A Local CLI Bridge on `http://127.0.0.1:3344` with the `pi` and `codex` backends ready.
...
| --- | --- |
| Same profile on Pi and Codex | Braid proofs `artifacts/verification/live-core/pi.json` and `codex.json` (2026-08-09). A 0.3.0 rerun in this branch stalled; rerun before recording. |

...
The setup view accepts endpoint, optional bearer credential reference, default execution mode, and health timeout.

The default local endpoint must be loopback and an unencrypted non-loopback endpoint is rejected unless an explicit trusted transport policy is configured.
...
| PC-12 | Removing a connection cannot silently cancel a run, destroy an environment, orphan an unacknowledged secret reference, or erase historical receipts. |

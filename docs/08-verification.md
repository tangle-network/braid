...
| --- | --- | --- |
| `repository` | `pnpm check` | Format, lint, strict types, boundaries, dependency/license metadata, and deterministic checks |
| `unit` | `pnpm test:unit` | Unit and normal property tests |
| `contract` | `pnpm test:contract` | Shared port and capability conformance |
| `rpc` | `pnpm test:rpc` | Packed-binary JSONL protocol tests |
| `virtual-terminal` | `pnpm test:virtual-terminal` | Cell, layout, Unicode, and state snapshots |
| `pty` | `pnpm test:pty` | Packed-binary real terminal keyboard and lifecycle tests |
| `storage` | `pnpm test:storage` | W5 storage verification; the W6 command fails explicitly unavailable until implemented |
| `security` | `pnpm test:security` | Current W6 secret redaction, terminal-control, path, and boundary checks |
| `performance` | `pnpm test:performance` | W12 PERF-01..10 measurements; the W6 command fails explicitly unavailable until the full matrix exists |
...
| `visual` | `pnpm capture:visual` | Deterministic real-binary captures and manifests |

...
It records the pending question, kills Braid, reconnects a fresh Braid process, and answers through `respond_interaction`.
...
The completed source run must report the same capability value.

When the capability is `true`, the proof creates a confidential workspace fork and requires a nonce-bound provider attestation.

The attestation must pass Braid's configured measurement, policy, provider-key, signature, environment, and request-digest checks.

The proof also rejects missing attestation, a wrong nonce, a wrong measurement, and self-echoed provider evidence.

When the capability is `false`, Braid must refuse the confidential plan and the real execute path with `CAPABILITY_UNAVAILABLE`.

That refusal must happen before a child environment, checkpoint, or fork operation is created.

...
export class SerializedEffectCoordinator {
  ...
  start<TRequest>(intent: EffectIntent<TRequest>, handler: EffectHandler<TRequest>): EffectHandle {
    ...
    if (!reservation.created && admitted.status !== 'pending') {
    ...
    if (!reservation.created) {
      ...
      const completion = this.#schedule(key, () =>
    ...
    const key = this.#key(intent.operationId, requestDigest)
    const completion = this.#schedule(key, () => this.#dispatchNew(intent, pending, handler))
    return {
      operationId: intent.operationId,
      requestDigest,
      replayed: false,
  ...
  #schedule(key: string, task: () => Promise<EffectRecord>): Promise<EffectRecord> {
    const completion = this.#serialize(task)

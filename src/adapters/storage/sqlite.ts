  ...
  implements StoragePort, EffectStoragePort
{
  ...
  static async open(options: SqliteStorageOptions): Promise<SqliteStorage> {
    ...
    let newDatabase: boolean
    try {
      newDatabase = isNewDatabaseFile(path)
    } catch (error) {
      if (
        markerState === 'stale' &&
        error instanceof StorageError &&
        error.code === 'STORAGE_CORRUPT_EMPTY'
      ) {
    ...
    const initializationClaimed = newDatabase || markerState === 'stale'
    if (initializationClaimed) await claimInitializationMarker(path)
    let databaseKey: Buffer
    try {
      databaseKey = await resolveDatabaseKey(options, databaseKeyRef)
    } catch (error) {
      if (initializationClaimed) {
        if (newDatabase) await releaseInitializationMarker(path).catch(() => undefined)
        else await markInitializationInterrupted(path).catch(() => undefined)
      }
      throw error
    }
    let database: SqliteDatabase | undefined
    try {
      database = databaseFactory(path, { timeout: busyTimeoutMs })
      configureCipherDatabase(database, databaseKey, { newDatabase })
      applyConnectionPragmas(database, busyTimeoutMs)
    } catch (error) {
      try {
        database?.close()
      } catch {
      ...
      databaseKey.fill(0)
      if (initializationClaimed) {
        if (newDatabase) await removeDatabaseArtifacts(path)
        await releaseInitializationMarker(path)
      }

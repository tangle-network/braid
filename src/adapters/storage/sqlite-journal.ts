...
export abstract class SqliteJournalStorage extends SqliteLifecycleStorage {
  ...
  async appendWithSnapshot(input: {
    readonly events: readonly JournalEvent[]
    readonly snapshot: StateSnapshot
  }): Promise<AppendResult> {
    this.validateAppend(input.events)
    return this.writes.run(async () => this.appendUnsafe(input.events, input.snapshot))
  }

  private validateAppend(events: readonly JournalEvent[]): void {
    if (events.length === 0) {
      return
    }
  ...
  async appendUnsafe(
    ...
    const createdSnapshotKeys: CredentialRef[] = []
    let preparedSnapshot: PreparedStateSnapshot | undefined
    try {
      for (const event of events) {
      ...
      if (snapshot !== undefined) {
        preparedSnapshot = await this.stateSnapshots.prepare(snapshot, createdSnapshotKeys)
      }
      this.begin()
      const acceptedEventIds: EventId[] = []
      const duplicateEventIds: EventId[] = []
      const gapRuns = new Set<RunId>()
      let snapshotStorageId: number | undefined
      let committed = false
      try {
        ...
        for (const event of events) {
          ...
          const storageId = asNumber(inserted.lastInsertRowid, 'storage_id')
          encoded.fill(0)
          if (snapshot?.eventId === event.eventId) snapshotStorageId = storageId
        ...
        if (snapshot !== undefined && acceptedEventIds.includes(snapshot.eventId)) {
          if (preparedSnapshot === undefined) {
            throw new StorageError('STATE_SNAPSHOT_INVALID', 'Snapshot key preparation was lost')
          }
          if (snapshotStorageId === undefined)
            throw new StorageError('STATE_SNAPSHOT_UNBOUND', 'Snapshot event is not stored')
          this.stateSnapshots.writeUnsafe(snapshot, snapshotStorageId, preparedSnapshot)
        }
        this.commit('append')
        committed = true
        createdKeys.length = 0
        if (snapshot !== undefined && acceptedEventIds.includes(snapshot.eventId)) {
          createdSnapshotKeys.length = 0
          await this.stateSnapshots.pruneAfterCommitUnsafe()
        } else if (createdSnapshotKeys.length > 0) {
          await this.stateSnapshots.discardCredentials(createdSnapshotKeys)
          createdSnapshotKeys.length = 0
        }
      ...
      if (error instanceof StorageError && error.code === 'CONTENT_KEY_RACE') {
        return this.appendUnsafe(events, snapshot)
      }
  ...
  async events(
    ...
    if (input.afterStorageId !== undefined) {
      clauses.push('storage_id > ?')
      parameters.push(input.afterStorageId)
    }

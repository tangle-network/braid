...
const INTENTS_FOR: Readonly<Record<CommandName, CommandIntentFactory>> = {
  ...
  graph: () => ({ type: 'open-surface', surface: 'graph' }),
...
const DEFINITIONS: readonly CommandDefinition[] = [
  ...
  ['runner', [], 'Set a runner for the next run', '/runner [name]', 'run.runner', 'explicit'],
  ['model', [], 'Set a model for the next run', '/model [name]', 'run.model', 'explicit'],
  [
    'effort',
    [],
    'Set reasoning effort for the next run',
    '/effort [level]',
    'run.effort',
    'explicit',
  ],
  ...
  ['graph', [], 'Open the conversation and run graph', '/graph', 'graph.read', 'none'],

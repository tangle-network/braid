...
function streamCandidateIsPrefix(value: string): boolean {
  return (
    ...
    STREAM_SECRET_NAMES.some((name) => streamCaseInsensitivePrefix(value, name)) ||
    STREAM_BARE_PREFIXES.some(({ prefix }) => prefix.startsWith(value))
  )
}

function streamAssignmentName(value: string): boolean {
  return STREAM_SECRET_NAMES.some((name) => name.toLowerCase() === value.toLowerCase())
}
...
/** Emits safe text promptly while retaining only an active control or secret candidate. */
export class IncrementalSecretTextSanitizer {
  ...
  finish(): string {
    ...
    const marker = SECRET_TRUNCATION_MARKER
    if (this.#truncated && this.#outputBytes < this.#maxOutputBytes) {
      this.#append(output, takeUtf8Prefix(marker, this.#maxOutputBytes - this.#outputBytes))
    }
    return output.join('')
  }
  ...
  #consumeCharacter(character: string, output: string[]): void {
    ...
    for (;;) {
      if (this.#state === 'normal') {
        if (streamCandidateIsPrefix(current)) {
          this.#state = 'candidate'
          this.#pending = current
          this.#pendingBytes = Buffer.byteLength(current, 'utf8')
          this.#assignmentEligible = isStreamAssignmentBoundary(this.#lastCharacter)
          return
        }
        this.#append(output, current)
      ...
      if (this.#state === 'candidate') {
        ...
        if (this.#pending.toLowerCase() === STREAM_BEARER_PREFIX) {
          if (/\s|=/u.test(current)) {
            this.#pending += current
            this.#pendingBytes += Buffer.byteLength(current, 'utf8')
      ...
      if (this.#state === 'assignment-value') {
        if (!this.#assignmentValueStarted) {
          ...
          if (current === '"' || current === "'") {
            this.#assignmentQuote = current
            this.#assignmentValueStarted = true
            return
          }
          if (isStreamAssignmentDelimiter(current)) {
        ...
        if (this.#assignmentQuote !== undefined) {
          if (current === this.#assignmentQuote) this.#resetRedactionState()
          return
        }
        if (isStreamAssignmentDelimiter(current)) {
          this.#resetRedactionState()
      ...
      if (this.#state === 'bearer-discard') {
        if (isStreamBearerDelimiter(current)) {
          this.#resetRedactionState()
          continue
        }
        return
      ...
      if (this.#state === 'bare-token') {
        if (isStreamBareCharacter(current)) {
          this.#pending += current
          this.#pendingBytes += Buffer.byteLength(current, 'utf8')
          this.#bareBodyBytes += Buffer.byteLength(current, 'utf8')
          if (

...
export function makeRecord(input: {
  ...
  const material: Omit<SemanticEvalRecord, 'artifactHash'> = {
    ...
    calibration: redactEvalArtifactValue(input.calibration) as CalibrationSummary,
    pilot: redactEvalArtifactValue(input.pilot) as SemanticEvalRecord['pilot'],
    cases: redactEvalArtifactValue(input.cases) as readonly SemanticCaseEvidence[],
    rawJudgeCalls: redactEvalArtifactValue(input.rawJudgeCalls) as readonly RecordedJudgeCall[],

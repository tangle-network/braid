...
export function environmentRecord({
  ...
  const details = {
    ...
    environment,
    boundary,
    machine: environmentDetails.machine ?? environment.BRAID_RELEASE_MACHINE ?? process.arch,
    os: environmentDetails.os ?? environment.BRAID_RELEASE_OS ?? process.platform,
    node: environmentDetails.node ?? process.version,
    region: environmentDetails.region ?? environment.BRAID_RELEASE_REGION ?? 'local',
    workspace:
      environmentDetails.workspace ?? environment.BRAID_RELEASE_WORKSPACE ?? 'isolated-release',
    packageVersions: environmentDetails.packageVersions ?? packageVersions,
    resourceIds: environmentDetails.resourceIds ?? resourceIds,
    billableResourceIds: environmentDetails.billableResourceIds ?? billableResourceIds,
  }
  return {
...
export function boundaryForCheck({ cwd, processResult, identity, requirementIds, resources = [] }) {
  return {
    schemaVersion: 1,
    shell: false,
    cwd,
...
export function buildCheckRecord({
  ...
  const boundary = boundaryForCheck({ cwd, processResult, identity, requirementIds, resources })
  return {
    id: checkId,
    category,
    required: true,

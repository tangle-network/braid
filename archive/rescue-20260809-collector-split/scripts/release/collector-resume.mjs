...
export function restorePassedChecks({
  ...
  for (const resource of envelope.liveResources) liveResources.set(resource.id, resource)
  for (const record of envelope.cleanup) cleanup.set(record.resourceId, record)
  for (const check of envelope.checks) {

  ...
  "environments": [
    {
      ...
      "details": {
        ...
        "packageVersions": {
          ...
          "@tangle-network/agent-runtime": "<exact>"
        },
        "resourceIds": [],
        "billableResourceIds": []
      }
...
Each fixed command row has one declared owning requirement in the release binding file, so a generic passing command cannot be reassigned to an unrelated requirement merely because its category is admissible.
...
Live checks name the resource identifiers used by the run, and each environment inventories those resources and the package versions present in that environment.

Every recorded resource has a confirmed cleanup record whose timestamp falls inside the release window.

The release cannot complete with unresolved cleanup, omitted resource inventory, unknown resource references, or incomplete machine and package provenance.

...
export async function runMatrixAdapter({ environment }) {
  ...
  return {
    status: 'unavailable',
    reason: configured
      ? 'External Tangle matrix adapters are not accepted as release proof; built-in parent checks for LIVE-09 through LIVE-10 are unavailable'
      : 'Built-in parent checks for LIVE-09 through LIVE-10 are unavailable',
  }
}
...
export async function runTangleFlows({
  ...
  try {
    const interactive = await interactiveRunner({ repository, environment, binary, invocationId })
    setFlow('LIVE-08', {
      row: 'LIVE-08',
    ...
    const classified = classifyExternalFailure(error, 'Tangle interaction', environment)

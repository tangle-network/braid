    ...
    assert.throws(
      ...
      (error) => {
        ...
        if (scope === 'live-tangle') {
          ...
          assert.match(
            output,
            /"row":"LIVE-08","status":"unavailable","reason":"BRAID_TANGLE_SANDBOX requires a protected credential/u,
          )
          assert.doesNotMatch(
            output,
            /"row":"LIVE-08","status":"unavailable","reason":"[^"]*BRAID_TANGLE_SANDBOX_(?:ENDPOINT|MODEL|RUNNER)/u,
          )
        }

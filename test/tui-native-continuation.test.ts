...
function finalEnvelope(input: ExecuteTurnInput, text: string): RuntimeEventEnvelope {
  return {
    runId: input.runId,
  ...
  const execution: ExecutionPort = {
    ...
    async *streamTurn(input): AsyncIterable<RuntimeEventEnvelope | RuntimeStreamEvent> {
      const controlRef = distinctControlRefs
        ...
        : CONTROL_REF
      const observed = kind === 'native' && input.nativeContextBoundaryProof === undefined
      if (observed) yield observedEvent(input, controlRef)
      if (terminal !== 'none')
        yield observed
          ? finalEnvelope(
              input,

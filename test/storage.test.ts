...
test('production SQLite retention, redaction rewrite, and content-key destruction remove readable history', async () => {
  ...
  const redactedEvent = journal.all()[0]?.event
  assert.equal(redactedEvent?.kind, 'content.unavailable')
  if (redactedEvent?.kind === 'content.unavailable') {
    assert.equal(redactedEvent.reason, 'redacted')
  }
  ...
  const deletedEvent = deletedJournal.all()[0]?.event
  assert.equal(deletedEvent?.kind, 'content.unavailable')
  if (deletedEvent?.kind === 'content.unavailable') {
    assert.equal(deletedEvent.reason, 'deleted')
  }

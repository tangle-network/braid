...
try {
  switch (action) {
    ...
    case 'destruction':
      ...
      break
    case 'snapshot-write':
      await storage.writeStateSnapshot(snapshotForStorage(storage, 'event-crash-1', 1))
      break
    case 'snapshot-prune': {
      if (storage.appendWithSnapshot === undefined)
        throw new Error('Production SQLite does not expose atomic snapshot append')
      await storage.appendWithSnapshot({
        events: [event(3, 'event-crash-3')],
        snapshot: snapshotForStorage(storage, 'event-crash-3', 3),
      })
      break
    }
    case 'snapshot-quarantine':
      await storage.latestStateSnapshot()
      break
